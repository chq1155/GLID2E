...
class Diffusion(L.LightningModule):
  ...
  def forward(self, x, sigma):
    ...
    return logits
  def value_forward(self, x, sigma):
    """Returns log score."""
    sigma = self._process_sigma(sigma)
  ...
  def _ddpm_update_finetune_rl(self, x, t, dt, return_process=False, old_model =None):
    ...
    # assert 0
    _x = _sample_categorical(q_xs)
    _x_argmax = q_xs[:, :, :-1].argmax(dim=-1)
    raw_x = _x.detach()
    log_p = q_xs.log().gather(2, _x.unsqueeze(-1)).squeeze(-1)
    if old_model is not None:
      old_log_p_x0 = old_model.forward(x, unet_conditioning)
      old_q_xs = old_log_p_x0.exp() * (move_chance_t-move_chance_s)
      old_q_xs[:, :, self.mask_index] = move_chance_s[:, :, 0]
      old_log_p = old_q_xs.log().gather(2, _x.unsqueeze(-1)).squeeze(-1)
    # _x = F.one_hot(_x, num_classes=self.vocab_size).to(torch.float32)
    # assert 0
    copy_flag = (x != self.mask_index).to(x.dtype)
    # print(x.shape, copy_flag.shape, _x.shape)

    if return_process:
      if old_model is None:
        return copy_flag * x + (1 - copy_flag) * _x, x, unet_conditioning, move_chance_t, copy_flag, log_p, raw_x
      else:
        return copy_flag * x + (1 - copy_flag) * _x, x, unet_conditioning, move_chance_t, copy_flag, log_p, old_log_p, raw_x, copy_flag * x + (1 - copy_flag) * _x_argmax
    else:
      return copy_flag * x + (1 - copy_flag) * _x
  ...
  # @torch.no_grad()
  def _sample_finetune_rl(self, num_steps=None, eps=1e-5, eval_sp_size=None, copy_flag_temp=None, old_model=None):
    """Generate samples from the model."""
    assert self.parameterization == 'subs' and self.sampler == 'ddpm'
    if eval_sp_size is None:
      batch_size_per_gpu = self.config.loader.eval_batch_size
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ddpm':
        # if i < num_steps - self.config.finetuning.truncate_steps:
        if old_model is None:
          x, last_x, condt, move_chance_t, copy_flag, log_p, raw_x = self._ddpm_update_finetune_rl(x, t, dt, return_process=True)
        else:
          x, last_x, condt, move_chance_t, copy_flag, log_p, old_log_p, raw_x, _ = self._ddpm_update_finetune_rl(x, t, dt, return_process=True, old_model=old_model)
          old_logp_sum += old_log_p.sum(dim=1)
        logp_sum += log_p.sum(dim=1)
        x = x.detach()
        copy_flag = copy_flag.unsqueeze(-1).detach()
        last_x = F.one_hot(last_x, num_classes=self.vocab_size).to(torch.float32).detach()
    ...
    x_argmax = torch.nn.functional.one_hot(x_argmax, num_classes=self.vocab_size-1).to(torch.float32)
    if old_model is not None:
      return x[:, :, :-1] + (x_argmax - x[:, :, :-1]).detach(), last_x_list, condt_list, move_chance_t_list, copy_flag_list, logp_sum, old_logp_sum, raw_x_list
    return x[:, :, :-1] + (x_argmax - x[:, :, :-1]).detach(), last_x_list, condt_list, move_chance_t_list, copy_flag_list, logp_sum, raw_x_list

  @torch.no_grad()
  def _sample_finetune_rl_old(self, num_steps=None, eps=1e-5, eval_sp_size=None, copy_flag_temp=None, old_model=None, reward_model=None, value_model=None):
    """Generate samples from the model."""
    assert self.parameterization == 'subs' and self.sampler == 'ddpm'
    ...
    for i in range(num_steps):
      ...
      if self.sampler == 'ddpm':
        ...
        assert sigma_t.ndim == 1, sigma_t.shape
        unet_conditioning = sigma_t
        value_list.append(value_model.value_forward(x, sigma_t))
        x, last_x, condt, move_chance_t, copy_flag, log_p, old_logp, raw_x, r_x = self._ddpm_update_finetune_rl(x, t, dt,return_process=True, old_model=old_model)
        
        logp_list.append(log_p)#.sum(dim=1)
        old_logp_sum += old_logp.sum(dim=1)
        x = x.detach()
        copy_flag = copy_flag.unsqueeze(-1).detach()
        last_x = F.one_hot(last_x, num_classes=self.vocab_size).to(torch.float32).detach()

      # sample_argmax = torch.argmax(sample, 2)
      sample_argmax = 1.0 * F.one_hot(r_x, num_classes= 4)
      sample_argmax = torch.transpose(sample_argmax, 1, 2)
      preds_argmax = reward_model(sample_argmax).squeeze(-1)
      reward_argmax = preds_argmax[:, 0]
      # logp_list.append(log_p)
      t_list.append(t)
      reward_list.append(reward_argmax)
      raw_x_list.append(raw_x)
      last_x_list.append(last_x)
      condt_list.append(condt)
      move_chance_t_list.append(move_chance_t)
    ...
    value_list.append(torch.zeros_like(value_list[0]))
    return x[:, :, :-1] + (x_argmax - x[:, :, :-1]).detach(), last_x_list, condt_list, move_chance_t_list, copy_flag_list, logp_list, old_logp_sum, raw_x_list, reward_list, input_x, value_list, t_list
  # def _ddpm_update_logp(self, x, t, dt, return_process=True):
